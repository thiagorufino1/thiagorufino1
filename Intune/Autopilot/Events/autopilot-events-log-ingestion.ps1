...
ForEach ($Detail in $AutopilotEvents.Value) {
    ...
    if ($deploymentEndDateTime -gt $dateTime24HoursAgo) {
        ...
        [string]$Target_Apps = $Detail.targetedAppCount
        [string]$Target_Policies = $Detail.targetedPolicyCount
        ...
        Add-Member -InputObject $Obj -MemberType NoteProperty -Name "Targeted_App_Count" -Value $Target_Apps
        Add-Member -InputObject $Obj -MemberType NoteProperty -Name "Targeted_Policy_Count" -Value $Target_Policies
...
if ($Devices_Details_Json.count -eq 1) {
    $Devices_Details_Json = "[$Devices_Details_Json]"
}
...
if ($Uploadbody) {
    ...
    Write-Output "Nenhum provisionamento nas últimas 24 horas."
}